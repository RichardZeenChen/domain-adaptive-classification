        ...
        with tf.Session() as sess:
            ...
            try:
                while not coord.should_stop():
                    ...
                    """checkpoint"""
                    if step % 1000 == 0:
                        saver.save(sess, os.path.join(training_path, FLAGS.naming + '.ckpt'), global_step=step) #'fast-style-model.ckpt'
            except tf.errors.OutOfRangeError:
                saver.save(sess, os.path.join(training_path, FLAGS.naming + '.ckpt-done')) # 'fast-style-model.ckpt-done'
