...
def get_dataset(dataset_name, root, source, target, train_source_transform, val_transform, train_target_transform=None):
    ...
    if dataset_name == "Digits":
        ...
        val_dataset = concat_dataset(root=root, tasks=target, download=False, transform=val_transform,
                                     start_idx=len(source))
        if dataset_name == 'DomainNet' or 'DomainChange' or 'EqualChange': # or 'Net':or 'EqualChange'
            test_dataset = concat_dataset(root=root, tasks=target, split='test', download=False, transform=val_transform,
                                          start_idx=len(source))
